...
class _UptProfileScreenState extends State<UptProfileScreen> {
  ...
  File? _imageFile;
  String? _currentPhotoBase64; // <-- Menggunakan Base64 string
  ...
  Future<void> _pickImage(ImageSource source) async {
    ...
    final XFile? image = await picker.pickImage(
      source: source,
      maxWidth: 400, // Diperkecil agar hemat ukuran teks base64
      maxHeight: 400,
      imageQuality: 50,
    );
    if (image != null) {
    ...
    if (user != null) {
      ...
      try {
        ...
        if (userDoc.exists) {
          ...
          _currentPhotoBase64 = data['photoUrl'] ?? ''; // Disimpan sebagai Base64
        }
      } catch (e) {
        // Abaikan
      }
  ...
  Future<void> _simpanPerubahan() async {
    ...
    try {
      ...
      // Jika ada file baru, konversi ke Base64
      if (_imageFile != null) {
      ...
      await FirebaseFirestore.instance.collection('users').doc(user!.uid).update({
        ...
        'photoUrl': finalImageBase64, // Simpan Teks Base64 langsung ke Firestore
      });
